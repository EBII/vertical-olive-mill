...
class FruitJuiceAnalysis(models.Model):
    _name = 'fruit.juice.analysis'
    _description = 'Fruit Juice Analysis'
    _order = 'id desc'
    _inherit = ['mail.thread']

    name = fields.Char(string='Analysis Number', index=True, readonly=True)
    juice_source_type = fields.Selection([
        ('arrival', 'Arrival'),
        ('tank', 'Tank'),
        ], default='arrival', string='Juice Source Type', required=True,
        track_visibility='onchange', states={'done': [('readonly', True)]})
    arrival_line_id = fields.Many2one(
        'fruit.arrival.line', string='Arrival Line',
    ...
    execution_user_id = fields.Many2one(
        ...
        states={'done': [('readonly', True)]}, track_visibility='onchange',
        default=lambda self: self.env.user.company_id.fruit_juice_analysis_default_user_id.id or False)
    ...
    line_ids = fields.One2many(
        'fruit.juice.analysis.line', 'analysis_id', string='Analysis Lines',
        states={'done': [('readonly', True)]})
    ...
    def create(self, vals):
        ...
        return super(FruitJuiceAnalysis, self).create(vals)

    @api.onchange('location_id')
    def location_id_change(self):
        if self.juice_source_type == 'tank' and self.location_id:
            self.season_id = self.location_id.fruit_season_id
            self.juice_product_id = self.location_id.juice_product_id
            # if juice tank is merged
    ...
    def arrival_line_id_change(self):
        if self.juice_source_type == 'arrival' and self.arrival_line_id:
            self.season_id = self.arrival_line_id.season_id
            self.juice_product_id = self.arrival_line_id.juice_product_id
            self.lot_id = False
    ...
    def print_report(self):
        action = self.env['report'].get_action(self, 'fruit.juice.analysis')
        return action

    @api.model
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(FruitJuiceAnalysis, self).fields_view_get(
            view_id=view_id, view_type=view_type, toolbar=toolbar, submenu=submenu)
        return self.env.user.company_id.current_season_update(res, view_type)


class FruitJuiceAnalysisLine(models.Model):
    _name = 'fruit.juice.analysis.line'
    _description = 'Fruit Juice Analysis Line'

    analysis_id = fields.Many2one(
        'fruit.juice.analysis', ondelete='cascade', string='Analysis')
    product_id = fields.Many2one(
        'product.product', string='Analysis Type',
        required=True, states={'done': [('readonly', True)]},
        domain=[('fruit_type', '=', 'analysis')])
    ...
    uom = fields.Char(
        related='product_id.fruit_analysis_uom', readonly=True,
        string='Unit of Measure')
    juice_source_type = fields.Selection(
        related='analysis_id.juice_source_type', readonly=True, store=True)
    juice_product_id = fields.Many2one(
        related='analysis_id.juice_product_id',
    ...
    def product_id_change(self):
        if self.product_id:
            self.instrument = self.product_id.fruit_analysis_instrument
            self.precision = self.product_id.fruit_analysis_precision
            self.decimal_precision = self.product_id.fruit_analysis_decimal_precision
    ...
    def create(self, vals):
        if vals.get('product_id'):
            ...
            vals.update({
                'instrument': product.fruit_analysis_instrument,
                'precision': product.fruit_analysis_precision,
                'decimal_precision': product.fruit_analysis_decimal_precision,
                })
        return super(FruitJuiceAnalysisLine, self).create(vals)
    ...
    @api.model
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(FruitJuiceAnalysisLine, self).fields_view_get(
            view_id=view_id, view_type=view_type, toolbar=toolbar, submenu=submenu)
        return self.env.user.company_id.current_season_update(res, view_type)
