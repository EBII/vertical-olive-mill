...
class StockProductionLot(models.Model):
    ...
    fruit_production_id = fields.Many2one(
        'fruit.juice.production', 'Fruit Juice Production', ondelete='restrict',
        readonly=True)

    @api.depends('name', 'expiry_date', 'fruit_production_id')
    def name_get(self):
        res = []
        for lot in self:
            dname = lot.name
            ...
            if lot.fruit_production_id:
                dname = '%s (%s)' % (dname, lot.fruit_production_id.farmers)
    ...
    def browse_recursive_tree(self, root_quant, lines):
        if (
                ...
                root_quant.product_id.fruit_type in ('juice', 'bottle_full')):
            lot = root_quant.lot_id
            if not lot:
                    raise UserError(_(
                        "The quant ID %d of the fruit juice product '%s' "
                        "is not linked to a lot !") % (
                            root_quant.id, root_quant.product_id.display_name))
            if lot.fruit_production_id:
                for line in lot.fruit_production_id.line_ids:
                    lines[line] = True
    ...
    def report_get_arrival_lines(self):
        self.ensure_one()
        if not self.product_id.fruit_type:
            raise UserError(_(
                "The product '%s' has no fruit type.")
                % self.product_id.display_name)
        if self.product_id.fruit_type not in ('bottle_full', 'juice'):
            raise UserError(_(
                "The product '%s' has an fruit type '%s'. This report is only "
                "for products with fruit type 'Full Juice Bottle' or "
                "Fruit Juice'.") % (
                    self.product_id.display_name, self.product_id.fruit_type))
        ...
        for line in tmp_list:
            if line.commercial_partner_id in res:
                ...
                res[line.commercial_partner_id]['subtotal'] += line.fruit_qty
            else:
                res[line.commercial_partner_id] = {
                    'lines': line,
                    'subtotal': line.fruit_qty,
                    }
