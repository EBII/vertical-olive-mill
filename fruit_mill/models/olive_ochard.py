...
class FruitOchard(models.Model):
    _name = 'fruit.ochard'
    _description = 'Fruit Ochard'
    _inherit = ['mail.thread']

    name = fields.Char(string='Name', required=True)
    partner_id = fields.Many2one(
        'res.partner', string='Fruit Farmer', ondelete='cascade', index=True,
        domain=[('parent_id', '=', False), ('fruit_farmer', '=', True)])
    active = fields.Boolean(default=True)
    zip_id = fields.Many2one('res.city.zip', 'City/Zip Shortcut')
    city = fields.Char('City')
    parcel_ids = fields.One2many(
        'fruit.parcel', 'ochard_id', string='Parcels')
