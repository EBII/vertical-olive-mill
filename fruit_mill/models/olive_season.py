...
class FruitSeason(models.Model):
    ...
    default_expiry_date = fields.Date(string='Default Juice Expiry Date')
    fruit_qty_arrived = fields.Integer(
        compute='_compute_totals', string='Arrived Fruit Qty (kg)', readonly=True)
    fruit_qty = fields.Integer(
        compute='_compute_totals', string='Pressed Fruit Qty (kg)', readonly=True)
    sale_juice_qty = fields.Integer(
        compute='_compute_totals', string='Sale Juice Qty (L)', readonly=True)
    juice_qty_with_compensation = fields.Integer(
        compute='_compute_totals', string='Juice Qty with Compensation (L)', readonly=True)
    withdrawal_juice_qty = fields.Integer(
        compute='_compute_totals', string='Withdrawal Juice Qty (L)', readonly=True)
    ...
    def _compute_totals(self):
        ...
        arrival_res = oalo.read_group([
            ...
            ('state', '=', 'done')],
            ['season_id', 'fruit_qty'], ['season_id'])
        for arrival_re in arrival_res:
            season = self.browse(arrival_re['season_id'][0])
            season.fruit_qty_arrived = arrival_re['fruit_qty']
        arrival_prod_done_res = oalo.read_group([
            ('season_id', 'in', self.ids),
            ('production_state', '=', 'done')],
            ['season_id', 'fruit_qty', 'juice_qty_with_compensation', 'sale_juice_qty', 'withdrawal_juice_qty'],
            ['season_id'])
        for re in arrival_prod_done_res:
            season = self.browse(re['season_id'][0])
            fruit_qty = re['fruit_qty']
            juice_qty_with_compensation = re['juice_qty_with_compensation']
            gross_ratio = 0
            if fruit_qty:
                gross_ratio = float_round(
                    100 * juice_qty_with_compensation / fruit_qty,
                    precision_digits=pr_ratio)
            season.fruit_qty = int(round(fruit_qty))
            season.juice_qty_with_compensation = int(round(juice_qty_with_compensation))
            season.gross_ratio = gross_ratio
            season.sale_juice_qty = int(round(re['sale_juice_qty']))
            season.withdrawal_juice_qty = int(round(re['withdrawal_juice_qty']))
    ...
    def _compute_kanban_dashboard_graph(self):
        for season in self:
            ...
            query = """
            SELECT date, sum(fruit_qty) as fruit_qty
            FROM fruit_arrival
            WHERE date >= %s
            AND date <= %s
            AND season_id = %s
            ...
            for sql_re in sql_res:
                date2qty[fields.Date.from_string(sql_re['date'])] = sql_re['fruit_qty']
    ...
    def dashboard_open_action(self):
        ...
        action = self.env['ir.actions.act_window'].for_xml_id(
            'fruit_mill', 'fruit_arrival_line_action')
        action.update({
            'context': {
    ...
    def generate_partner_organic_certif(self):
        ...
        for rg_re in rg_res:
            ...
            if partner.active and partner.fruit_farmer and not partner.parent_id:
