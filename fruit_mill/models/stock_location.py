...
class StockLocation(models.Model):
    ...
    fruit_tank_type = fields.Selection([
        ('regular', 'Regular'),
        ('compensation', 'Compensation'),
        ('shrinkage', 'Shrinkage'),
        ('risouletto', 'Risouletto'),
        ], string='Fruit Juice Tank Type')
    fruit_season_id = fields.Many2one(
        'fruit.season', string='Fruit Season', ondelete='restrict')
    fruit_season_year = fields.Char(
        related='fruit_season_id.year', store=True)
    juice_product_id = fields.Many2one(
        'product.product', string='Juice Product',
        domain=[('fruit_type', '=', 'juice')])
    fruit_shrinkage_juice_product_ids = fields.Many2many(
        'product.product', 'stock_location_shrinkage_juice_product_rel',
        'location_id', 'product_id', domain=[('fruit_type', '=', 'juice')],
        string='Allowed Juice Products')
    fruit_juice_qty = fields.Float(
        compute='_compute_fruit_juice_qty', readonly=True,
        string='Fruit Juice Qty (L)',
        digits=dp.get_precision('Product Unit of Measure'))

    def _compute_fruit_juice_qty(self):
        ...
        for rg_re in rg_res:
            ...
            qty = loc.fruit_tank_type and rg_re['qty'] and float_round(rg_re['qty'], precision_digits=prec) or 0
            loc.fruit_juice_qty = qty

    @api.onchange('fruit_tank_type')
    def fruit_tank_type_change(self):
        if self.fruit_tank_type:
            if not self.fruit_season_id:
                season = self.env['fruit.season'].get_current_season()
                if season:
                    self.fruit_season_id = season
        else:
            self.fruit_season_id = False
    ...
    def name_get(self):
        ...
        for entry in res:
            ...
            new_name = entry[1]
            if loc.fruit_tank_type and loc.juice_product_id:
                new_name = '%s (%s, %s)' % (
                    new_name, loc.fruit_season_year, loc.juice_product_id.name)
            new_res.append((entry[0], new_name))
        return new_res

    def fruit_juice_tank_compatibility_check(self, juice_product, season):
        """This method should be called AFTER fruit_juice_tank_check()
        so we don't re-do the checks made in fruit_juice_tank_check()"""
        self.ensure_one()
        if self.fruit_tank_type != 'risouletto':
            if self.juice_product_id != juice_product:
                raise UserError(_(
                    "You are working with juice product '%s', "
                    "but the fruit tank '%s' is configured "
                    "with juice product '%s'.") % (
                        juice_product.display_name,
                        self.name,
                        self.juice_product_id.display_name))
            if self.fruit_season_id != season:
                raise UserError(_(
                    "You are working with fruit season '%s', "
                    "but the fruit tank '%s' is configured "
                    "with fruit season '%s'.") % (
                        season.name,
                        self.display_name,
                        self.fruit_season_id.name))

    def fruit_juice_tank_check(self, raise_if_not_merged=True, raise_if_empty=True):
        '''Returns quantity
        ...
        prec = self.env['decimal.precision'].precision_get('Product Unit of Measure')
        tank_type = self.fruit_tank_type
        tank_type_label = dict(self.fields_get('fruit_tank_type', 'selection')['fruit_tank_type']['selection'])[tank_type]
        # Tank configuration checks
        if not tank_type:
            raise UserError(_(
                "The stock location '%s' is not an fruit juice tank.")
                % self.display_name)
        if not self.juice_product_id:
            raise UserError(_(
                "Missing juice product on tank '%s'.") % self.name)
        if self.juice_product_id.fruit_type != 'juice':
            raise UserError(_(
                "Juice product '%s' configured on tank '%s' is not "
                "an fruit juice product.") % (
                    self.juice_product_id.display_name, self.name))
        if tank_type != 'risouletto' and not self.fruit_season_id:
            raise UserError(_(
                "Fruit season is not configured on tank '%s'.") % self.name)
        ...
        if tank_type == 'risouletto':
            for quant_product in quant_product_rg:
                ...
                if raise_if_not_merged and product != self.juice_product_id:
                    raise UserError(_(
                        "The tank '%s' (type '%s') contains '%s', "
                        "so it not merged.") % (
                            self.name, tank_type_label, product.display_name))
                if product.fruit_type != 'juice':
                    raise UserError(_(
                        "The tank '%s' (type '%s') contains '%s', "
                        "which is not an fruit juice product.") % (
                            self.name, tank_type_label, product.display_name))
        else:  # regular juice => always 1 product, same as configured on tank
            if len(quant_product_rg) > 1:
                raise UserError(_(
                    "There are several different products in tank '%s'. "
                    "This should never happen in an juice tank which is "
                    "not a risouletto tank.") % self.name)
            product = ppo.browse(quant_product_rg[0]['product_id'][0])
            if product != self.juice_product_id:
                raise UserError(_(
    ...
    def fruit_juice_transfer(
        ...
        if dest_loc == self:
            raise UserError(_(
                "You are trying to transfer juice from '%s' to the same location!")
                % self.display_name)
        sqo = self.env['stock.quant']
        ...
        src_qty = src_loc.fruit_juice_tank_check(
            raise_if_not_merged=raise_if_not_merged)
        # compat src/dest
        if dest_loc.fruit_tank_type:
            dest_loc.fruit_juice_tank_check(
                raise_if_not_merged=False, raise_if_empty=False)
            dest_loc.fruit_juice_tank_compatibility_check(
                src_loc.juice_product_id, src_loc.fruit_season_id)
        ...
        if transfer_type == 'full':
            ...
            if float_compare(partial_transfer_qty, 0, precision_digits=pr_juice) <= 0:
                raise UserError(_(
                    "The quantity to transfer (%s L) must be strictly positive.")
                    % partial_transfer_qty)
            if float_compare(partial_transfer_qty, src_qty, precision_digits=pr_juice) >= 0:
                raise UserError(_(
                    "The quantity to transfer (%s L) from tank '%s' is superior "
                    "to its current juice quantity (%s L).") % (
                        partial_transfer_qty, src_loc.name, src_qty))
            product = src_loc.juice_product_id
            mvals = {
                'name': _('Partial juice tank transfer'),
                'origin': origin,
