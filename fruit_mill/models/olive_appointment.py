...
class FruitAppointment(models.Model):
    _name = 'fruit.appointment'
    _description = 'Fruit Appointment'
    ...
    partner_id = fields.Many2one(
        'res.partner', string='Fruit Farmer', required=True, copy=False,
        domain=[('fruit_farmer', '=', True)], track_visibility='onchange')
    ...
    fruit_culture_type = fields.Selection(
        related='partner_id.commercial_partner_id.fruit_culture_type',
        readonly=True)
    ...
    variant_id = fields.Many2one(
        'fruit.variant', string='Fruit Variant', track_visibility='onchange')
    juice_product_id = fields.Many2one(
        'product.product', string='Juice Type',
        domain=[('fruit_type', '=', 'juice')])
    qty = fields.Integer(
        string='Quantity', help="Fruit quantity in kg",
    ...
    juice_destination = fields.Selection([
        ...
        ('sale', 'Sale'),
        ('mix', 'Mix'),
        ], string='Juice Destination', track_visibility='onchange')
    withdrawal_juice_qty = fields.Integer(
        string='Withdrawal Juice Quantity', track_visibility='onchange')
    ...
    total_qty_same_day = fields.Integer(
        ...
        string='Total Fruit Arrival Same Day', readonly=True,
        help="Total qty of fruit arrival on the same day, including this appointment.")
    ...
    _sql_constraints = [(
        ...
        'withdrawal_juice_qty_positive',
        'CHECK(withdrawal_juice_qty >= 0)',
        'The quantity of juice withdrawn must be positive or null.'),
        ]
    ...
    @api.depends(
        'partner_id', 'appointment_type', 'qty', 'juice_destination',
        'withdrawal_juice_qty',
        'juice_product_id', 'withdrawal_invoice',
    ...
    def _compute_display_calendar_label(self):
        for app in self:
            ...
            if app.fruit_culture_type and app.fruit_culture_type != 'regular':
                fruit_culture_type_label = dict(app.fields_get('fruit_culture_type', 'selection')['fruit_culture_type']['selection'])[app.fruit_culture_type]
                label += ' [%s]' % fruit_culture_type_label
            if app.appointment_type in ARRIVAL_TYPES:
                label += ', %d kg' % app.qty
                if app.juice_destination == 'withdrawal':
                    label += _(' (Withdrawal)')
                elif app.juice_destination == 'mix':
                    label += _(' (Partial withdrawal: %d L)') % app.withdrawal_juice_qty
                if app.juice_product_id:
                    label += ', %s' % app.juice_product_id.name
    ...
    def juice_destination_change(self):
        if self.juice_destination in ('sale', 'withdrawal'):
            self.withdrawal_juice_qty = 0
        ...
        if self.start_datetime and self.appointment_type and self.company_id:
            ...
            if self.appointment_type in ARRIVAL_TYPES:
                if self.appointment_type == 'arrival_leaf_removal':
                    duration_coef = company.fruit_appointment_arrival_leaf_removal_minutes
                else:
                    duration_coef = company.fruit_appointment_arrival_no_leaf_removal_minutes
                # math.ceil() rounds up: math.ceil(5.1) = 6.0
                minutes = int(math.ceil(duration_coef * self.qty / 100.0))
                if minutes < company.fruit_appointment_arrival_min_minutes:
                    minutes = company.fruit_appointment_arrival_min_minutes
            elif self.appointment_type == 'lend':
                minutes = company.fruit_appointment_lend_minutes
            elif self.appointment_type in ('withdrawal', 'other'):
                minutes = company.fruit_appointment_withdrawal_minutes
            if minutes:
    ...
    def _compute_palox_qty(self):
        for app in self:
            palox_qty = 0.0
            qty_per_palox = app.company_id.fruit_appointment_qty_per_palox
            if qty_per_palox:
    ...
    def open_arrival(self):
        ...
        context = {
            ...
            'default_default_juice_destination': self.juice_destination,
            'default_default_juice_product_id': self.juice_product_id.id or False,
            }
        if self.appointment_type == 'arrival_leaf_removal':
            context['default_default_leaf_removal'] = True
        action = self.env['ir.actions.act_window'].for_xml_id(
            'fruit_mill', 'fruit_arrival_action')
        action.update({
            'view_mode': 'form,tree',
            'views': False,
            'context': context,
    ...
    def open_new_appointment(self):
        ...
        context = {
            ...
            'default_juice_destination': self.juice_destination,
            'default_leaf_removal': self.leaf_removal,
            'default_juice_product_id': self.juice_product_id.id or False,
            'default_qty': self.qty,
            'default_withdrawal_juice_qty': self.withdrawal_juice_qty,
            }
        action = self.env['ir.actions.act_window'].for_xml_id(
            'fruit_mill', 'fruit_appointment_action')
        action.update({
    ...
    def show_arrival_appointments_same_day(self):
        self.ensure_one()
        action = self.env['ir.actions.act_window'].for_xml_id(
            'fruit_mill', 'fruit_appointment_tree_action')
        action.update({
            'views': False,
            'context': {},
            })
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(FruitAppointment, self).fields_view_get(
