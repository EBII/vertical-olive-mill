...
class FruitLendedCase(models.Model):
    _name = 'fruit.lended.case'
    _description = 'Fruit Lended Cases'
    _order = 'date desc'

    partner_id = fields.Many2one(
        'res.partner', string='Fruit Farmer',
        domain=[('parent_id', '=', False), ('fruit_farmer', '=', True)],
        ondelete='restrict', index=True)
    fruit_culture_type = fields.Selection(
        related='partner_id.fruit_culture_type', readonly=True)
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(FruitLendedCase, self).fields_view_get(
