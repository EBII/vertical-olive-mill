# Copyright 2019 Barroux Abbey (https://www.barroux.org/)
# @author: Alexis de Lattre <alexis.delattre@akretion.com>
# License AGPL-3.0 or later (http://www.gnu.org/licenses/agpl).

from odoo import fields, models


class FruitAppointmentPrint(models.TransientModel):
    _name = 'fruit.appointment.print'
    _description = 'Wizard to print the appointments of the day'

    date = fields.Date(
        string='Date', default=fields.Date.context_today, required=True)

    def report_get_appointments(self):
        self.ensure_one()
        appointments = self.env['fruit.appointment'].search(
            [('date', '=', self.date)], order='start_datetime')
        return appointments

    def run(self):
        self.ensure_one()
        action = self.env['report'].get_action(self, 'fruit.appointment.day')
        return action
