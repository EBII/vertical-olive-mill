...
class FruitJuiceTankMerge(models.TransientModel):
    _name = 'fruit.juice.tank.merge'
    _description = 'Wizard to merge an fruit juice tank'
    ...
    def validate(self):
        ...
        origin = _('Fruit juice tank merge wizard')
        ppo = self.env['product.product']
        mpo = self.env['mrp.production']
        splo = self.env['stock.production.lot']
        sqo = self.env['stock.quant']
        loc = self.location_id
        qty = loc.fruit_juice_tank_check(raise_if_not_merged=False)
        # Check if already merged
        error_msg = _(
            "Juice tank '%s' is already merged.") % loc.name
        quant_lot_rg = sqo.read_group(
            [('location_id', '=', loc.id)],
            ['qty', 'lot_id'], ['lot_id'])
        if self.location_id.fruit_tank_type == 'risouletto':
            if len(quant_lot_rg) == 1:
                lot = splo.browse(quant_lot_rg[0]['lot_id'][0])
                if lot.product_id == loc.juice_product_id:
                    raise UserError(error_msg)
        else:
            if len(quant_lot_rg) == 1:
                raise UserError(error_msg)
        product = loc.juice_product_id
        assert product.tracking == 'lot'
        assert product.fruit_type == 'juice'
        ...
        if len(boms) != 1:
            raise UserError(_(
                "The juice product '%s' should only have a single bill "
        ...
        for bom_line in bom.bom_line_ids:
            if bom_line.product_id.fruit_type != 'juice':
                raise UserError(_(
                    "The bill of material of the juice product '%s' "
                    "should only have juice components.")
        ...
        if float_compare(bom_lines_qty, bom.product_qty, precision_digits=p_prec):
            raise UserError(_(
                "The bill of material of the juice product '%s' is wrong: "
        ...
        if loc.fruit_tank_type != 'risouletto':
            if len(mo.move_raw_ids) != 1:
                raise UserError(_(
                    "Wrong bill of material for fruit product '%s' "
                    "configured on juice tank '%s': it should have a single "
            ...
            if mo.move_raw_ids[0].product_id != product:
                raise UserError(_(
                    "Wrong bill of material for fruit product '%s' "
                    "configured on juice tank '%s': it's single component "
        ...
        if mo.availability != 'assigned':
            raise UserError(_(
                "Could not reserve the juice to merge the tank %s. "
        ...
        # Create finished lot
        merge_lot_name = self.env['ir.sequence'].next_by_code('fruit.juice.merge.lot')
        new_lot = splo.create({
            'product_id': product.id,
            'name': merge_lot_name,
            })
        ...
        mo.button_mark_done()
        post_mo_qty = loc.fruit_juice_tank_check()
        if float_compare(qty, post_mo_qty, precision_digits=pr_juice):
            raise _(
                "In tank '%s', the juice quantity after the merge (%s) "
                "is different from the juice quantity before the merge (%s). "
                "This should never happen.")
