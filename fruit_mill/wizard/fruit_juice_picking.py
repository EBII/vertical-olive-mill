...
class FruitJuicePicking(models.TransientModel):
    _name = 'fruit.juice.picking'
    _description = 'Wizard to ship loose fruit juice'
    ...
    juice_qty = fields.Float(
        string='Juice Qty', readonly=True,
    ...
    src_location_id = fields.Many2one(
        'stock.location', string='Fruit Tank', required=True)
    ...
    container_ids = fields.One2many(
        'fruit.juice.picking.container', 'wizard_id', 'Containers Used')
    ...
    def validate(self):
        ...
        origin = _('Ship loose juice wizard')
        if self.picking_id:
            origin = _(
                'Ship loose juice from picking %s') % self.picking_id.name
        ...
        if self.move_id and self.move_id.state != 'confirmed':
            raise UserError(_(
                ...
                "state.") % (
                    ...
                    self.move_id.id,
                    self.move_id.state))
        if float_compare(self.juice_qty, 0, precision_digits=pr_prod) <= 0:
            raise UserError(_(
                "The quantity of fruit juice must be positive."))
        ...
        juice_start_qty_in_tank = self.src_location_id.fruit_juice_tank_check(
            raise_if_empty=True)
        # Check we have enough juice (and tank is merged)
        if float_compare(
                juice_start_qty_in_tank, self.juice_qty,
        ...
        name = _('Prepare loose fruit juice: %s') % self.juice_product_id.name
        juice_move = smo.create({
            'picking_id': opicking.id,
            'product_id': self.juice_product_id.id,
            'product_uom': self.juice_product_id.uom_id.id,
            'location_id': self.src_location_id.id,
            'location_dest_id': self.dest_location_id.id,
            'product_uom_qty': self.juice_qty,
            'name': name,
            'origin': origin,
            })
        # Move containers too
        ...
        if (
            ...
            for cline in self.container_ids:
                ...
                name = _(
                    'Container to prepare loose fruit juice: '
            ...
            if cpicking.state != 'done':
                raise UserError(_(
                    "Could not move the containers for this loose fruit "
                    "juice preparation."))
        ...
        if opicking.state != 'assigned':
            raise UserError(_(
                "Could not reserve the stock for this loose fruit juice "
                "preparation. Check that you have enough juice."))
        opicking.action_pack_operation_auto_fill()
        opicking.do_new_transfer()
        assert opicking.state == 'done'
        assert juice_move.state == 'done'

        # Check juice end qty
        juice_end_qty_in_tank = self.src_location_id.fruit_juice_tank_check(raise_if_empty=False)
        if float_compare(
                juice_end_qty_in_tank, juice_start_qty_in_tank - self.juice_qty,
                precision_digits=pr_juice):
            raise UserError(_(
                "The end quantity in tank (%s L) is wrong. This should never "
                "happen.") % juice_end_qty_in_tank)

        # Add containers to picking
        if self.container_ids and self.picking_id:
            for cline in self.container_ids:
                name = _(
                    'Container for loose fruit juice: '
                    '%s') % cline.product_id.name
                smo.create({
        ...
        if self.move_id:
            if sqo.search([('reserved_quantity', '!=', 0.0)]):
                raise UserError(_(
                    ...
                    "there was some unreserved juice left on stock "
            ...
            for quant in juice_move.quant_ids:
                quant.sudo().reserved_quantity = self.move_id.product_qty
            if self.picking_id:
                self.picking_id.action_assign()
                if self.picking_id.fruit_juice_picking_wizard_next_move_id:
                    action = self.picking_id.start_fruit_juice_picking_wizard()

        return action


class FruitJuicePickingContainer(models.TransientModel):
    _name = 'fruit.juice.picking.container'
    _description = 'Loose Fruit Juice: Container Lines'

    wizard_id = fields.Many2one('fruit.juice.picking', ondelete='cascade')
