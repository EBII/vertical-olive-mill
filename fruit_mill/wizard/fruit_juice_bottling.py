...
JUICE_QTY_NOT_EMPTY = 0.1


class FruitJuiceBottling(models.TransientModel):
    _name = 'fruit.juice.bottling'
    _description = 'Wizard to fill-up fruit juice bottles'

    # START first 1st step (select)
    bottle_product_id = fields.Many2one(
        'product.product', string='Juice Bottle to Produce',
        domain=[('fruit_type', '=', 'bottle_full')], required=True,
        readonly=True, states={'select': [('readonly', False)]})
    warehouse_id = fields.Many2one(
        'stock.warehouse', string='Warehouse',
        domain=[('fruit_mill', '=', True)], required=True,
    ...
    src_location_start_qty = fields.Float(
        string='Juice Qty in Tank before Bottling',
        digits=dp.get_precision('Product Unit of Measure'), readonly=True)
    juice_qty = fields.Float(
        string='Juice Qty for Bottling',
        digits=dp.get_precision('Product Unit of Measure'), readonly=True)
    src_location_end_qty = fields.Float(
        string='Juice Qty in Tank after Bottling',
        digits=dp.get_precision('Product Unit of Measure'), readonly=True,
    ...
    inventory_start_qty = fields.Float(
        string='Inventory Juice Qty in Tank before Bottling',
        digits=dp.get_precision('Product Unit of Measure'), readonly=True)
    inventory_end_qty = fields.Float(
        string='Inventory Juice Qty in Tank after Bottling',
        digits=dp.get_precision('Product Unit of Measure'), readonly=True)
    ...
    def select2qty(self):
        ...
        bom, juice_product_id, bottle_volume = self.bottle_product_id.juice_bottle_full_get_bom_and_juice_product()
        assert juice_product_id.fruit_type == 'juice'
        self.write({
            'state': 'qty',
            'bom_id': bom.id,
            'juice_product_id': juice_product_id.id,
        ...
        action = self.env.ref('fruit_mill.fruit_juice_bottling_action').read()[0]
    ...
    def qty2produce(self):
        ...
        src_location_start_qty = self.src_location_id.fruit_juice_tank_check()
        # Check we have enough juice
        juice_qty = self.bottle_qty * self.bottle_volume
        src_location_end_qty = float_round(src_location_start_qty - juice_qty, precision_digits=prec)
        inventory_required = 'no'
        inventory_start_qty = inventory_end_qty = 0
        fcompare = float_compare(src_location_start_qty, juice_qty, precision_digits=prec)
        if fcompare < 0:
            if self.src_location_end_status == 'empty':
                inventory_required = 'yes'
                inventory_start_qty = juice_qty
                inventory_end_qty = 0
            elif self.src_location_end_status == 'not_empty':
                inventory_required = 'yes'
                inventory_start_qty = juice_qty + JUICE_QTY_NOT_EMPTY
                inventory_end_qty = JUICE_QTY_NOT_EMPTY
        elif fcompare == 0:
    ...
    def validate(self):
        ...
        prec = self.env['decimal.precision'].precision_get('Product Unit of Measure')
        origin = _('Fruit juice bottling wizard')
        ...
        assert juice_product.tracking == 'lot'
        ...
        # Re-check the qty of juice in tank (could have been changed after validation
        # of last step
        src_location_start_qty = self.src_location_id.fruit_juice_tank_check()
        ...
        if float_compare(src_location_start_qty, self.src_location_start_qty, precision_digits=prec):
            raise UserError(_(
                "The quantity of juice in tank has changed between the "
        ...
        if self.inventory_required == 'yes':
            ...
            assert tank_quants.product_id == juice_product
            assert tank_quants.lot_id
            juice_lot_id = tank_quants.lot_id.id
            inv_line_vals = {
                'product_id': juice_product.id,
                'product_uom_id': juice_product.uom_id.id,
                'location_id': self.src_location_id.id,
                'prod_lot_id': juice_lot_id,
                'product_qty': self.inventory_start_qty,
                'theoretical_qty': src_location_start_qty,
                }
            inventory = self.env['stock.inventory'].create({
                'name': _('Juice bottling %s from %s') % (self.bottle_product_id.name, self.src_location_id.name),
                'location_id': self.src_location_id.id,
                'product_id': juice_product.id,
                'lot_id': juice_lot_id,
                'filter': 'none',
        ...
        juice_raw_move = smo.search([
            ('product_id.fruit_type', '=', 'juice'),
        ...
        for rmove in other_raw_moves:
            if rmove.product_id.tracking in ('lot', 'serial'):
                raise UserError(_(
                    ...
                    "of the bill of material tracked by lot is the juice.")
                    % rmove.product_id.display_name)
        # BOM has already been checked, so this should really never happen
        assert len(juice_raw_move) == 1, 'Wrong number of juice raw moves'
        ...
        if mo.availability != 'assigned':
            raise UserError(_(
                ...
                "Check that you have enough juice and empty bottles."))
        for move_lot in juice_raw_move.move_lot_ids:
        ...
        for raw_move_lot in juice_raw_move.move_lot_ids:
            assert not raw_move_lot.lot_produced_id
        juice_raw_move.move_lot_ids.write({'lot_produced_id': bottle_lot.id})
        mo.write({
            'state': 'progress',
        ...
        # Check juice end qty
        juice_end_qty_in_tank = self.src_location_id.fruit_juice_tank_check()
        if float_compare(
                juice_end_qty_in_tank,
        ...
        if (
                ...
                not float_is_zero(juice_end_qty_in_tank, precision_digits=prec)):
            raise UserError(_(
                "The end quantity in tank (%s L) is wrong. It should be 0.") % juice_end_qty_in_tank)
