...
class FruitPaloxCaseLend(models.TransientModel):
    _name = 'fruit.palox.case.lend'
    _description = 'Wizard to lend palox and/or cases'

    arrival_id = fields.Many2one(
        'fruit.arrival', string='Arrival', readonly=True)
    way = fields.Selection([
        ('lend', 'Lend'),
        ('return', 'Return'),
        ], default='lend', required=True, string='Type', readonly=True)
    partner_id = fields.Many2one(
        'res.partner', string='Fruit Farmer', required=True, readonly=True,
        domain=[('fruit_farmer', '=', True), ('parent_id', '=', False)])
    lend_palox_ids = fields.Many2many(
        'fruit.palox', 'fruit_palox_wizard_lend_rel', 'wizard_id', 'palox_id',
        string='Palox')
    return_palox_ids = fields.Many2many(
        'fruit.palox', 'fruit_palox_wizard_return_rel', 'wizard_id', 'palox_id',
        string='Palox')
    fruit_culture_type = fields.Selection(
        related='partner_id.fruit_culture_type',
        readonly=True)
    ...
    def default_get(self, fields_list):
        res = super(FruitPaloxCaseLend, self).default_get(fields_list)
        if res['way'] == 'return' and res['partner_id']:
            palox = self.env['fruit.palox'].search([('borrower_partner_id', '=', res['partner_id']), ('borrowed_date', '!=', False)])
            if palox:
    ...
    def validate(self):
        ...
        if self.regular_case_qty > 0 or self.organic_case_qty > 0:
            ...
            self.env['fruit.lended.case'].create({
